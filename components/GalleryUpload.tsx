  ...
  const handleSubmit = async () => {
    ...
    for (const selectedFile of selectedFiles) {
      try {
        ...
        formData.append("folder", "acda/gallery");
        ...
        const dbResponse = await fetch("/api/gallery/upload", {
          method: "POST",
          headers: {
    ...
    if (errorCount === 0) {
      setUploadStatus({
        type: "success",
        message: `All ${successCount} image(s) uploaded successfully!`,
      });
      ...
      setUploadStatus({
        type: "error",
        message: `Failed to upload all ${errorCount} image(s)`,
      });
    } else {
      setUploadStatus({
        type: "error",
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-lg">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-primary-800">Upload to Gallery</h2>
          <p className="text-sm text-gray-600">Add multiple images to the public gallery</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-1">
            ...
            <label
              htmlFor="file-input"
              className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8 transition-colors hover:border-primary-400 hover:bg-primary-50 ${
                isUploading ? "cursor-not-allowed opacity-50" : ""
              }`}
            >
              <ImageIcon className="mb-3 h-16 w-16 text-gray-400" />
              <p className="text-base font-medium text-gray-700">Click to select images</p>
              <p className="mt-2 text-sm text-gray-500">PNG, JPG, WEBP up to 10MB each</p>
              <p className="mt-1 text-sm text-gray-500">Select multiple files at once</p>
            </label>
          </div>
        </div>
